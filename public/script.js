...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─── 2. Field Note Dialog ───
  const fieldNoteDialog = document.getElementById('fieldNoteDialog');
  const fieldNoteOpenButtons = document.querySelectorAll('[data-field-note-open]');
  const fieldNoteCloseButtons = document.querySelectorAll('[data-field-note-close]');
  let lastFieldNoteTrigger = null;

  if (fieldNoteDialog && typeof fieldNoteDialog.showModal === 'function') {
    fieldNoteOpenButtons.forEach(button => {
      button.addEventListener('click', () => {
        lastFieldNoteTrigger = button;
        fieldNoteDialog.showModal();
        document.body.classList.add('dialog-open');
      });
    });

    fieldNoteCloseButtons.forEach(button => {
      button.addEventListener('click', () => fieldNoteDialog.close());
    });

    fieldNoteDialog.addEventListener('click', (event) => {
      if (event.target === fieldNoteDialog) {
        fieldNoteDialog.close();
      }
    });

    fieldNoteDialog.addEventListener('close', () => {
      document.body.classList.remove('dialog-open');
      if (lastFieldNoteTrigger) {
        lastFieldNoteTrigger.focus();
      }
    });
  }
